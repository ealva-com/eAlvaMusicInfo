 ...
 */
@SuppressWarnings("unused") public interface Throttle {
   ...
   *
   * @return time spent sleeping to enforce rate, in seconds; 0.0 if not rate-limited
   * @throws ThrottleException if this Thread is interrupted.  The cause is set to the caught
   ...
   * @return {@code true} if the permits were acquired, {@code false} otherwise
   * @throws IllegalArgumentException if the requested number of permits is negative or zero
   * @throws RuntimeException if this Thread is interrupted.  The cause is set to the caught
   * InterruptedException and this Thread is re-interrupted
   */
