...
abstract class NanoThrottle implements Throttle {
  ...
  private NanoThrottle(final double permitsPerSecond, final boolean fair) {
    if (permitsPerSecond <= 0.0 || doubleIsInvalid(permitsPerSecond)) {
  ...
  private boolean doubleIsInvalid(final double permitsPerSecond) {
    return Double.isInfinite(permitsPerSecond) || Double.isNaN(permitsPerSecond);
  }
